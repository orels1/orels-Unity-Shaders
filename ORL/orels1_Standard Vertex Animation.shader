...
Shader "orels1/Standard Vertex Animation"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			void VertexAnimationVertex() {
				half3 wsVert = TransformObjectToWorld(vD.vertex.xyz);
				half3 wsNormal = TransformObjectToWorld(vD.normal.xyz);
				half factor = 0;
				
				#if defined(SPIN_LOCAL) || defined(SPIN_WORLD)
				half3 origin = _SpinOrigin.xyz;
				#if defined(SPIN_LOCAL)
				vD.vertex.xyz = RotateAroundAxis(origin, vD.vertex.xyz, _SpinAxis.xyz, _Time.y * _SpinSpeed);
				vD.normal.xyz = RotateAroundAxis(origin, vD.normal.xyz, _SpinAxis.xyz, _Time.y * _SpinSpeed);
				...
				half3 vertRot = RotateAroundAxis(wsCenter, wsCenter + vD.vertex.xyz * scale, _SpinAxis.xyz, _Time.y * _SpinSpeed);
				half3 normalRot = RotateAroundAxis(wsCenter, wsNormal, _SpinAxis.xyz, _Time.y * _SpinSpeed);
				
				vD.vertex.xyz = TransformWorldToObject(half4(vertRot, 1)).xyz;
				vD.normal.xyz = TransformWorldToObject(half4(normalRot, 1)).xyz;
				#endif
				wsVert = TransformObjectToWorld(vD.vertex.xyz);
				#endif
				...
				#endif
				vD.vertex.xyz += factor * axis * _FloatAmount;
				wsVert = TransformObjectToWorld(vD.vertex.xyz);
				#endif
				
				#if defined(SCALE_UNIFORM)
				factor = sin(_Time.y * _ScaleSpeed);
				vD.vertex.xyz += factor * vD.vertex.xyz * _ScaleAmount;
				wsVert = TransformObjectToWorld(vD.vertex.xyz);
				#endif
			}
